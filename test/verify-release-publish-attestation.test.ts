...
test("an unknown array reference is left in place rather than silently dropped", () => {
  // expandArrays uses `??` to fall back to the original text when an array name
  // is not declared. Silently dropping it would turn "this scan does not understand
  // the command" into "this command has no flags", which reads as a pass.
  const result = auditPublishAttestation([
    { file: "release.yml", text: `          npm publish \${unknown[@]}` },
  ]);
  assert.equal(result.failures.length, 1);
});

test("an escaped hash inside a publish segment is not treated as a comment start", () => {
  // stripComment sees a backslash and skips the next character, so an escaped #
  // is a literal argument, not a comment. Without this the command would be
  // truncated at the # and the flag after it would be lost.
  const result = auditPublishAttestation([
    { file: "release.yml", text: `          ${ATTESTED}\\#not-a-comment` },
  ]);
  assert.deepEqual(result.failures, []);
});

test("a single quote inside a double-quoted argument does not open a single-quoted span", () => {
  // stripComment tracks quote state to know whether a # is inside quotes. A
  // single quote inside a double-quoted string must not toggle the single flag;
  // otherwise a # later in the string would be misread as a comment start.
  const result = auditPublishAttestation([
    { file: "release.yml", text: `          ${ATTESTED}  "it's here"` },
  ]);
  assert.deepEqual(result.failures, []);
});

test("a single-quoted argument outside double quotes is tracked by stripComment", () => {
  // stripComment must toggle the single flag when it sees a lone single quote,
  // so a # inside a single-quoted argument is not mistaken for a comment.
  const result = auditPublishAttestation([
    { file: "release.yml", text: `          ${ATTESTED}  'some arg'` },
  ]);
  assert.deepEqual(result.failures, []);
});
