...
names = [s["name"] for s in test_steps]
install_idx = names.index("Install clone-local pm merge drivers")
health_idx = names.index("Verify tracked pm project health")
assert install_idx == health_idx - 1, "merge driver install must run before the health gate"
health_run = test_steps[health_idx]["run"]
assert "--require-merge-drivers" in health_run, "health gate missing --require-merge-drivers"

print("release.yml alert job and ci.yml merge-driver gate verified")
