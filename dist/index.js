...
export function buildBeadsFromWorkspace(pmRoot, opts) {
...
export function normalizeDiffField(bead, field) {
    switch (field) {
        case "title":
...
export function changedFields(a, b) {
    ...
    for (const field of DIFF_FIELDS) {
        if (normalizeDiffField(a, field) !== normalizeDiffField(b, field))
            out.push(field);
    }
    return out;
}
...
export function indexBeadsById(beads) {
    const index = new Map();
...
export function diffBeads(a, b, filter) {
    const apply = (list) => {
        if (!filter || (!filter.statuses && !filter.types))
            return list;
...
function printDiffSummary(diff, labelA, labelB) {
    ...
    if (diff.changed.length) {
        ...
        for (const c of diff.changed)
            console.error(`    ~ ${c.id} (${c.fields.join(", ")})`);
    }
...
function runDiff(args, opts) {
    ...
    if (opts.againstWorkspace) {
        if (files[1]) {
            throw new CommandError("Provide exactly one file with --against-workspace (the second source is the current workspace).", EXIT_CODE.USAGE);
        }
        if (!opts.pmRoot) {
        ...
        const fileB = files[1];
        if (!fileB) {
            throw new CommandError("Provide two files, or one file with --against-workspace. Usage: pm beads diff <fileA> <fileB>", EXIT_CODE.USAGE);
        }
        beadsA = parseBeadsFileForDiff(fileA);
    ...
    if (opts.strict && diff.drift) {
        throw new CommandError(`Drift detected: ${diff.added.length} added, ${diff.removed.length} removed, ${diff.changed.length} changed.`, EXIT_CODE.GENERIC_FAILURE);
    }
    return { a: labelA, b: labelB, ...diff };
}
// ---------------------------------------------------------------------------
...
export function parseFilterExpression(raw) {
    const filter = { statuses: undefined, types: undefined };
    if (!raw)
        return filter;
...
export function mergeRowFilters(base, override) {
    return {
...
export function parseMergeStrategy(options) {
    const raw = optionString(options, "merge-strategy", "mergeStrategy");
    if (raw === undefined)
        return "update";
    const v = raw.toLowerCase();
    if (MERGE_STRATEGIES.includes(v))
        return v;
    throw new CommandError(`Unknown --merge-strategy "${raw}". Valid values: ${MERGE_STRATEGIES.join(", ")}.`, EXIT_CODE.USAGE);
}
...
export function parseDiffOptions(options, global = {}, pmRoot) {
    return {
...
export default defineExtension({
    ...
    activate(api) {
        ...
        // -----------------------------------------------------------------------
        const makeDiffCommand = (name) => ({
            name,
            description: "Compare two Beads JSONL files (or a file against the current pm workspace) and report per-bead " +
